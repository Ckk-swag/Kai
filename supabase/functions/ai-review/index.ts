...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const text = await requestAiReview(settings, body.reviewData);
...
async function requestAiReview(settings: AiSettings, reviewData: unknown) {
  ...
  const res = await fetch(endpoint, {
    ...
    body: JSON.stringify({
      ...
      messages: [
        { role: 'system', content: '你是一个温柔但具体的博士成长教练。请基于用户提供的结构化数据，用中文输出简洁、可执行的复盘。' },
        { role: 'user', content: `请分析这些成长数据，输出：1) 今日/近期模式洞察；2) 最大卡点；3) 明天最优先的3个行动；4) 鼓励但不空泛的一句话。数据：${JSON.stringify(reviewData)}` },
      ],
      temperature: 0.7,
    }),
  });
